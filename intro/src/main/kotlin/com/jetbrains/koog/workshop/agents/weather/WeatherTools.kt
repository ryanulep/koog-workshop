...
class WeatherTools(
  ...
  suspend fun weatherForecast(
    @LLMDescription("The location to get the weather forecast for (e.g., 'London, UK')")
    location: String,
    // The date to get the weather forecast for in ISO format (e.g., '2023-05-20'). If empty,
    // the forecast starts from today.
    @LLMDescription(
      "The date to get the weather forecast for in ISO format (e.g., '2023-05-20'). If empty, the forecast starts from today."
    )
    date: String = "",
    @LLMDescription("The number of days to get the weather forecast for (1-7). Defaults to 1.")
    days: Int = 1,
    @LLMDescription("The granularity of the weather forecast (daily or hourly). Defaults to daily.")
    ...
    val formattedLocation =
      if (loc.country.isNullOrBlank()) loc.name else "${loc.name}, ${loc.country}"
  ...
  suspend fun addDatetime(
    @LLMDescription("The date to add to in ISO format (e.g., '2023-05-20')") date: String,
    @LLMDescription("The number of days to add") days: Int,
    @LLMDescription("The number of hours to add") hours: Int,
    @LLMDescription("The number of minutes to add") minutes: Int,
  ): String {
    val baseDate =
      if (date.isNotBlank()) {
        try {
    ...
    val resultDate =
      baseInstant.plus(period, defaultTimeZone).toLocalDateTime(defaultTimeZone).date.toString()
  ...
  private fun formatHourlyForecast(forecast: WeatherForecast, date: String): String {
    ...
    val startIndex =
      hourly.time.indexOfFirst { it.startsWith(startDate) || it > startDate }.coerceAtLeast(0)

    return buildString {
      val groupedByDate =
        hourly.time
          .subList(startIndex, hourly.time.size)
          .mapIndexed { index, time ->
            ...
            val precipProb =
              hourly.precipitationProbability?.getOrNull(actualIndex)?.toString() ?: "N/A"
