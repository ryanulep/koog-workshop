...
class CharacterRepository {
    ...
    suspend fun getCharacter(id: Long): Character? = transaction {
    ...
    suspend fun createCharacter(name: String): Long = dbQuery {
        Characters.insertAndGetId {
            it[Characters.name] = name
        }.value
    }

    suspend fun getWalletBalance(characterId: Long): Map<Long, Long> = dbQuery {
        Transactions.select(Transactions.character)
